...
public class GUIDataCollector: MonoBehaviour
{
    ...
    // After the game starts it is updated with the data set by the user.
    ...
    private void SetPanelData(GameObject panel, int id, bool enabled)
    {
        ...
        FindObject("PlayerKeyLeftButton", id).GetComponentInChildren<Text>().text = keyCodesMap[configurator.Players[id - 1].MovementKeys[0].ToString()];
        FindObject("PlayerKeyRightButton", id).GetComponentInChildren<Text>().text = keyCodesMap[configurator.Players[id - 1].MovementKeys[1].ToString()];
    }
    ...
    private void Start()
    {
        ...
        arenaSizeSlider.onValueChanged.AddListener(delegate { UpdateArenaSize(arenaSizeSlider); });
        ...
        playersSpeedSlider.onValueChanged.AddListener(delegate { UpdatePlayersSpeed(playersSpeedSlider); });
        ...
        playersSizeSlider.onValueChanged.AddListener(delegate { UpdatePlayersSize(playersSizeSlider);  });
